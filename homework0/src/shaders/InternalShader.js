...
vNormal = normalize(uNormalMatrix * aNormalPosition) ;
...
vec3 lightOffset = uLightPos - vFragPos;
float lightDistance = length(lightOffset);
vec3 lightDir = lightOffset / max(lightDistance, 0.0001);
vec3 normal = normalize ( vNormal );
float diff = max(dot(lightDir , normal), 0.0);
float lightAttenuation = uLightIntensity / max(lightDistance, 0.0001);
vec3 diffuse = diff * lightAttenuation * color;

vec3 viewDir = normalize ( uCameraPos - vFragPos );
vec3 halfwayDir = normalize(lightDir + viewDir);
float spec = diff > 0.0 ? pow(max(dot(normal, halfwayDir), 0.0), 35.0) : 0.0;
